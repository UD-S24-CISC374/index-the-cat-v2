...
export default class TargetIndex extends Phaser.GameObjects.Container {
    ...
    constructor(scene: Phaser.Scene, x: number, y: number) {
        ...
        let box = this.scene.add
            .rectangle(0, 0, 260, 100, 0x000000)
            .setOrigin(0.5, 0.5)
            .setStrokeStyle(5, 0xfff);
        this.add(box);

        // Make the text
        this.targetIndexText = scene.add
            .text(0, 0, "")
            .setColor("white")
            .setFontSize(80)
            .setOrigin(0.5, 0.5);
    ...
    updateIndex(level: Level, listLength: number) {
        ...
        let normalizedIndex = this.normalizeIndexOrSubscript(
            indexes,
            listLength
        );
