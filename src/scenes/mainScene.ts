...
export default class MainScene extends Phaser.Scene {
    ...
    /**
     * What mode we are currently playing in.
     */
    private mode: string;
    /**
    ...
    create() {
        ...
        this.listGroup = new NumberList(
            this,
            centerX,
            this.scale.height + this.cat.CAT_BODY_OFFSET
        );
     ...
     */
    startRound() {
        let level = PLAYABLE_LEVELS[this.mode][this.level];
        ...
        this.listGroup.resetNumbers(
            catBodyCount,
            this.cat.CAT_BODY_WIDTH,
            level.generateValue
        );
    ...
    endPet(mouse: Phaser.Input.Pointer, gameObject: NumberList) {
        ...
        if (chosen !== this.dragger.endIndex) {
            ...
            if (moved && this.dragger.endIndex !== null) {
                this.listGroup.highlightRange(
                    this.dragger.startIndex,
                    this.dragger.endIndex
                );
            }
        }
    ...
    endPetOutside(mouse: Phaser.Input.Pointer) {
        ...
        let normalized = this.targetIndex.normalizeIndexOrSubscript(
            indexes as Subscript,
    ...
    private handlePet(correct: boolean) {
        ...
        this.feedbackTimer = this.time.addEvent({
            ...
            callback: () => {
                ...
                this.checkRound();
                this.cat.resetHead();
                this.targetIndex.updateIndex(
                    PLAYABLE_LEVELS[this.mode][this.level],
                    this.listGroup.length
                );
